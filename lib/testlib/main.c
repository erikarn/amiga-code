...
#define DEVICE_NAME "testlib.library"
#define DEVICE_DATE "(1 Sep 2020)"
#define DEVICE_ID_STRING "testlib " XSTR(DEVICE_VERSION) "." XSTR(DEVICE_REVISION) " " DEVICE_DATE /* format is: 'name version.revision (d.m.yy)' */
...
init_device(BPTR seg_list asm("a0"), struct Library *dev asm("d0"))
{
    ...
    dev->lib_Node.ln_Type = NT_LIBRARY;
...
expunge(struct Library *dev asm("a6"))
{
#if DEBUG
...
open(struct Library *dev asm("a6"), struct IORequest *ioreq asm("a1"),
...
static BPTR __attribute__((used))
close(struct Library *dev asm("a6"), struct IORequest *ioreq asm("a1"))
{
#if DEBUG
...
static void __attribute__((used))
begin_io(struct Library *dev asm("a6"), struct IORequest *ioreq asm("a1"))
{
#if DEBUG
...
abort_io(struct Library *dev asm("a6"), struct IORequest *ioreq asm("a1"))
{
#if DEBUG
...
// Note; lib base is always in a6, that's why it's specified like this
static void __attribute__((used))
test_func(struct Library *dev asm("a6"))
{
}

// ok, where's my LVO for all of this? I'll have to write it.
